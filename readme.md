...
- **Backend:** [Cloudflare Workers](https://workers.cloudflare.com/) + [Hono](https://hono.dev/) (TypeScript)
- **База данных:** [Cloudflare D1](https://developers.cloudflare.com/d1/) (Serverless SQLite)
- **Файловое хранилище:** [Cloudflare R2](https://developers.cloudflare.com/r2/) (хранение фото конспектов)
- **Frontend:** [React 18](https://react.dev/), [TypeScript](https://www.typescriptlang.org/), [Vite](https://vitejs.dev/), [React Router v6](https://reactrouter.com/)
...
1. **Видеоурок:** Видеозапись с YouTube (поддерживаются форматы `youtube.com/watch?v=...`, `youtu.be/...`, `shorts`, `embed`).
2. **Конспект:** Текстовая инструкция, что именно ученик должен записать в тетрадь.
3. **Тест / Задачи:** Ссылка на Google Docs / Google Forms с вопросами теста для самопроверки.
4. **Видеоразбор теста:** YouTube-видео с подробным объяснением решений.
5. **Зона отправки конспекта:** Загрузка фотографий тетради ученика (до 10 фото, загрузка в Cloudflare R2).

### 🔒 Система прогресса и блокировки уроков
- Первый урок первого раздела открыт сразу после одобрения регистрации ученика.
- Урок $N+1$ **заблокирован** до тех пор, пока ученик не загрузит фото конспекта по уроку $N$ и учительница не поставит статус **«Успех»**.
...
В репозитории нет и не должно быть общего пароля учительницы. Единственная
администраторская учётная запись создаётся автоматически при первом входе из
двух секретов Cloudflare. Публичная регистрация создаёт только учеников.
...
После production-деплоя войдите на `/login` с этими email и паролем. При первом входе
создастся аккаунт с ролью `teacher`; все страницы `/admin` и API админ-панели
остаются недоступны ученикам. Смена `TEACHER_PASSWORD` безопасно сменит пароль
учительницы при следующем входе.
...
Если база была создана до этого обновления, один раз примените ограничение на
единственный аккаунт учительницы. При первом входе старый демонстрационный
email и пароль будут заменены значениями из секретов:
```bash
npx wrangler d1 execute fizika-lab-db --remote --file=migrations/0001_single_teacher.sql
```

Чтобы применить защиту от повторной отправки конспекта к уже созданной базе,
выполните также:
```bash
npx wrangler d1 execute fizika-lab-db --remote --file=migrations/0002_one_active_conspect.sql
```

Чтобы включить перевод учеников между классами с сохранением доступа к прошлым
материалам, примените также:
```bash
...
2. **Заявки и доступы учеников (`/admin/requests`):**
...
3. **Проверка конспектов (`/admin/conspects`):**
...
4. **Управление темами и уроками (`/admin/classes`):**
